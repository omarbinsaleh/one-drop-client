      ...
      <nav id='navbar' className='bg-white/60 dark:bg-slate-800/60 shadow-md text-black p-3 py-2 sticky top-0 left-0 backdrop-blur-xl'>
            ...
            </div>
            {
               user ? <div className='hidden md:flex items-center gap-2'>
                  ...
                  <div className='dropdown dropdown-end relative'>
                     <div onClick={() => setShowProfileSubMenu(!showProfileSubMenu)} role="button" className="btn btn-ghost btn-circle avatar">
         ...
         <div className={` ${showLink ? 'block animate__animated animate__fadeInDown' : 'hidden animate_animated animate_fadeInUp'} pt-3`}>
            <ul className='pb-4 space-y-3 text-lg dark:text-white'>
               ...
               <li>
                  {user ? <button onClick={handleLogout} className='btn btn-md btn-block bg-opacity-80 border-none bg-secondary text-white rounded-sm  flex items-center justify-center'> <VscSignOut className='text-[17px]' /> Sign out</button> : <Link to='/auth/sign-in' onClick={() => setShowLink(false)} className='btn bg-opacity-50 border-none bg-secondary rounded-sm flex items-center justify-center' > <VscSignIn className='text-[17px]' /> Sign In</Link>}
               </li>
            </ul>
         </div>
      </nav>
